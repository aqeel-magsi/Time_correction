...
for ii = 10
    ...
    for i = 1:lengday
        ...
        for j = -3:3 %%
            ...
            if k <1; k = 1; end
           % zz = CFdata(k).NCF(2000:2360,1)'./max(CFdata(k).NCF(2000:2360,1));
     ...
     z2s=z2s/max(abs(z2s));
      Z = sprintf('NCF_%d.txt',sg);
     dlmwrite(Z,z2,'delimiter',' ')
      F = sprintf('Day_%d.txt',sg);
     dlmwrite(F,day,'delimiter',' ')
      T = sprintf('time_%d.txt',sg);
     dlmwrite(T,time,'delimiter',' ')
     fig=figure(ii);
     h1=subplot(3,1,1);
     imagesc(time,day,z2);
     colormap(jet)
     ...
     %yval = ylim(gca);
     L=round(linspace(1,size(YTickStr,1),8));
